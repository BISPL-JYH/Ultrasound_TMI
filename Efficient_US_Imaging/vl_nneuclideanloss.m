function Y = vl_nneuclideanloss(X, c, dzdy)
...
if nargin == 2 || (nargin == 3 && isempty(dzdy))
    ...
    Y = 1 / 2 * sum(subsref((X - c) .^ 2, substruct('()', {':'}))) / numel(X); % Y is divided by d(4) in cnn_train.m / cnn_train_mgpu.m.
...
elseif nargin == 3 && ~isempty(dzdy)
    ...
    Y = dzdy * (X - c) / numel(X); % Y is divided by d(4) in cnn_train.m / cnn_train_mgpu.m.
